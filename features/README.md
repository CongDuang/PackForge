...
- [x] `pnpm install && pnpm dev` 可打开「匠包」
- [x] 主路径：选项目 → 选模块/变体 → 选 JDK → 绑定/带出本工程签名 → 打包 → 日志 → 复制产物
- [x] 缺 Wrapper / SDK / JDK / 构建失败时错误码与人话提示符合 PRD §15
- [x] 密码不进明文 store；日志与命令预览脱敏
- [x] 当前开发机 OS 上 `electron-builder` 能产出可安装包并启动
- [x] Windows NSIS 配置已写好（若在 mac 开发，文档注明交叉验证步骤）
- [x] 本索引「可发布」勾选
